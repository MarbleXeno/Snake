...
{
	mSnakeCells.push_back(Cell(CellType::SNAKE_HEAD, { initCell.GetCellGridPosition().x, initCell.GetCellGridPosition().y }, {initCell.GetFloatRect().left, initCell.GetFloatRect().height * (initCell.GetCellGridPosition().y), initCell.GetFloatRect().width, initCell.GetFloatRect().height}, sf::Color::Black));
	mSnakeCells.back().SetCellMovingDirection(CellMovingDirection::UP);

	mSnakeCells.push_back(Cell(CellType::SNAKE_BODY, { initCell.GetCellGridPosition().x, initCell.GetCellGridPosition().y + 1 }, { initCell.GetFloatRect().left, initCell.GetFloatRect().height * (initCell.GetCellGridPosition().y + 1), initCell.GetFloatRect().width, initCell.GetFloatRect().height }, sf::Color::Black));
	mSnakeCells.back().SetCellMovingDirection(CellMovingDirection::UP);

	mSnakeCells.push_back(Cell(CellType::SNAKE_TAIL, { initCell.GetCellGridPosition().x, initCell.GetCellGridPosition().y + 2}, { initCell.GetFloatRect().left, initCell.GetFloatRect().height * (initCell.GetCellGridPosition().y + 2), initCell.GetFloatRect().width, initCell.GetFloatRect().height }, sf::Color::Black));
	mSnakeCells.back().SetCellMovingDirection(CellMovingDirection::UP);
...
gm::Cell gm::Snake::Move(Cell& nextCell, const bool& removeTail)
{
	...
	mSnakeCells.front().SetColor(sf::Color::Black);
	...
	mSnakeCells.back().SetColor(sf::Color::Black);
...
{
	nextCell.SetCellMovingDirection(mNextCellMovingDirection);
	Move(nextCell, false);
	mSnakeScore++;
}
